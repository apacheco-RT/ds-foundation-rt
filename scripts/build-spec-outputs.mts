...
async function loadVelite(collection: string) {
...
async function run() {
  ...
  await writeFile(
    resolve(ROOT, '.claude/registry-context/registry.json'),
    JSON.stringify({
      ...
      foundations: foundations.map((f: Record<string, unknown>) => ({
  ...
  for (const c of components as Record<string, unknown>[]) {
    ...
    const a11y = c['accessibility'] as Record<string, unknown>;
    ...
    const md = [
      ...
      `- **Role:** \`${a11y?.['role']}\``,
      `- **WCAG:** ${(a11y?.['wcag'] as string[])?.join(', ')}`,
      `- **Required ARIA:** ${(a11y?.['aria'] as string[])?.join(', ')}`,
  ...
  for (const c of components as Record<string, unknown>[]) {
    const figma = c['figma'] as Record<string, unknown> | undefined;
    if (!figma?.['code-connect']) continue;

    const id = c['id'] as string;
    const variants = (c['variants'] as string[]) ?? [];
    const sizes = (c['sizes'] as string[]) ?? [];
    const frameworks = c['frameworks'] as Record<string, string>;
    const nodeId = figma['node-id'] as string;
  ...
  await writeFile(
    resolve(ROOT, 'mcp/ds-server/src/registry-snapshot.json'),
    JSON.stringify({ components, foundations }, null, 2)
  );
