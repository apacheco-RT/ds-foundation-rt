...
export type AdapterTheme =
  ...
  | { type: 'js'; content: object }
  | { type: 'scss'; content: string };
...
export function createSystem(config: {
  ...
  return {
    ...
    resolveClass: (component: string, variant: string, size?: string) =>
      config.adapter.resolveClass(component, variant, size),
    resolveToken: (path: string) =>
      config.adapter.resolveToken(path, 'css-var'),
  };
}
